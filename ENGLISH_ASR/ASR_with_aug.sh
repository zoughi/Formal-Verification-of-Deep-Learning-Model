...
id=3 #here we detrmine where to save the output of this program (eg. $saved_run/data$id)
stage=0 #4
stage_StatTrain_HMM_NN=0 #9
...
if [ $stage -le 1 ]; then
      ...
      echo ============================================================================
      echo "         augment some part of Train data which store on Excel files            "
      echo ============================================================================
      for x in $MyDataBase_path/EXCEL/*IT*; do
         echo $x
         ssconvert $x/lable.xlsx $x/file.csv
         ./run1.sh $x tmp
      done      
      cd $Timit_path
      which_part=tmp
      ./myaugment.sh $id $stage_NewRun_Hiva 14000 $Myscript_path/for_Finglish $stage_run_tdnn_lstm_lfr_1a $which_part #run this line for new data
      [ -d $saved_run/mfcc$id ] && rm -rf $saved_run/mfcc$id
      [ -d $saved_run/exp$id ] && rm -rf $saved_run/exp$id
...
if [ $stage -le 4 ]; then
    ...
    ./StartTrain_HMM_NN.sh $id $stage_StatTrain_HMM_NN 100000 $Myscript_path/for_Finglish $stage_run_tdnn_lstm_lfr_1a  #run this line for new data
fi
