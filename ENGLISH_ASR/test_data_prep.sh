...
text_dir=$MyDataBase_path/Test_excel_conv/
wav_dir=$MyDataBase_path/Test_excel_conv/
...
trap 'rm -rf "$tmpdir"' EXIT
  ...
  ls -d "$*"/Test_excel_conv/* | sed -e "s:^.*/::" > $tmpdir/test_spk
...
for x in test; do
  ...
  # Create wav.scp
  awk '{printf("%s sox -v 0.70 %s -t wav -r 16000 -c 1 - |\n", $1, $2);}' < ${x}_sph.scp > ${x}_wav.scp
